...
const LPGError = require("./helpers/LPG-Error");
...
class Model {
  ...
  async get(primary_key) {
    ...
    if (!results) throw new LPGError("No User Found", 404);
  ...
  async update(primary_key, data) {
    ...
    if (result.rowCount === 0) {
      throw new LPGError(`No item with ${this.primary_key}: ${primary_key}`, 400);
    };
    return result.rows[0];
  };
